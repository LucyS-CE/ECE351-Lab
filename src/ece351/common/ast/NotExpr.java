...
public final class NotExpr extends UnaryExpr{
    ...
    protected final Expr simplifyOnce() {		
		...
		final Expr simplifiedExpr = expr.simplify();
		// !true = false
		if (simplifiedExpr instanceof ConstantExpr && ((ConstantExpr) simplifiedExpr).b) {
		...
		// !!x = x
		if (simplifiedExpr instanceof NotExpr) {
			return ((NotExpr) simplifiedExpr).expr;
		}
		// nothing changed
		...
		// something changed
			...
			return new NotExpr(simplifiedExpr);
		}
    }
