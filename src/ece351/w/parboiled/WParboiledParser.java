...
public /* final */ class WParboiledParser extends BaseParser351 {
    ...
    public Rule Program() {
        ...
        return Sequence(push(new WProgram()), OneOrMore(Waveform()), EOI); // 1. [->WProgram]
// throw new ece351.util.Todo351Exception();
    ...
    public Rule Waveform() {
        ...
        return Sequence(
                W0(), Name(),
                push(new Waveform(match())), // 2. [->Waveform(Name), WProgram]
                W0(), ':', W0(), BitString(), W0(), ';', W0(), // 3. do something in BitString()
                swap(), // 4. [WProgram, Waveform(Name)]
                push(((WProgram) pop()).append((Waveform) pop())) // 5. WProgram<-[Waveform(Name)]
                                                                  // 6. Waveform(Name)<-[]
                                                                  // 7. [->WProgram.append(Waveform(Name))]
        );
// throw new ece351.util.Todo351Exception();
    }
    ...
    public Rule Name() {
        // TODO: short code snippet
        return Sequence(W0(), OneOrMore(Letter()));
// throw new ece351.util.Todo351Exception();
    }
...
// throw new ece351.util.Todo351Exception();
    ...
    public Rule BitString() {
        // TODO: short code snippet
        return Sequence(W0(), OneOrMore(Sequence(Bit(), W0())));
// throw new ece351.util.Todo351Exception();
    }
    ...
    public Rule Bit() {
        ...
        return Sequence(AnyOf("01"),
                push(match()), // 3.1. [Bit, Waveform(Name), WProgram]
                swap(), // 3.2. [Waveform(Name), Bit, WProgram]
                push(((Waveform) pop()).append((String) pop())) // 3.3. Waveform(Name)<-[Bit, WProgram]
                                                                // 3.4. Bit<-[WProgram]
                                                                // 3.5. [->Waveform(Name).append(Bit), WProgram]
        );
// throw new ece351.util.Todo351Exception();
    }
