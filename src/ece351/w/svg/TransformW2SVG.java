...
public final class TransformW2SVG {
	...
	public static void transform(final WProgram wp, final PrintWriter out) {
		...
		out.println(
				"<style type=\"text/css\"><![CDATA[line{stroke:#006600;fill:#00 cc00;} text{font-size:\"large\";font-family:\"sans-serif\"}]]></style>");
		out.println();
		...
		for (final Waveform w : wp.waveforms) {
			...
			for (final String bit : w.bits) {
				...
				out.println(Line.toSVG(x, y_prev, x, y_pos)); // draw y
				out.println(Line.toSVG(x, y_pos, x + WIDTH, y_pos)); // draw x
				x += WIDTH;
				y_prev = y_pos;
