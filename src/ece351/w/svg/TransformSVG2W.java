...
public final class TransformSVG2W {
	 ...
	 * local variable is not final: just re-assign the local variable to the new
	 * list.
	 * 
	...
	public static final WProgram transform(final PinsLines pinslines) {
		...
		ArrayList<Waveform> wave_lists = new ArrayList<Waveform>();
		int count = pins.size(); // get number of pins
		pins.sort(COMPARE_Y); // sort pins by y position
		lines.sort(COMPARE_X); // sort lines by x position

		for (int i = 0; i < count; i++) {	
			Waveform w = transformLinesToWaveform(lines, pins.get(i)); // transform lines to waveform
			wave_lists.add(w); // add waveform to list
		}
		ImmutableList<Waveform> waveforms = ImmutableList.copyOf(wave_lists);
	 ...
	 * The concept of a y-midpoint might be useful: 1 is a line above; 0 is a line
	 * below.
	...
	private static Waveform transformLinesToWaveform(final List<Line> lines, final Pin pins) {
		...
		int y_mid = pins.y; // get y midpoint
		for (Line l : lines) {
			if (l.x1 == l.x2)
				continue; // skip vertical line)
			if (l.y1 + y_off == y_mid && l.y2 + y_off == y_mid) {
				int x_len = Math.abs(l.x1 - l.x2);
				int n = Math.max(1, x_len / WIDTH);
				for (int i = 0; i < n; i++) { // handle long lines
					bits.add("1");
				}
			} else if (l.y1 - y_off == y_mid && l.y2 - y_off == y_mid) {
				int x_len = Math.abs(l.x1 - l.x2);
				int n = Math.max(1, x_len / WIDTH);
				for (int i = 0; i < n; i++) { // handle long lines
					bits.add("0");
		...
		return new Waveform(ImmutableList.copyOf(bits), pins.id);
		// throw new ece351.util.Todo351Exception();
	}

	/**
	 * Sort a list of lines according to their x position. x1 first, and then x2.
	 * 
	...
	/**
	 * Sort a list of lines according to their y position first, and then x position
	 * second.
	 * 
	 * @see java.util.Comparator
	 */
	...
	// Simpler way to write the comparator using lambda expression
	// public final static Comparator<Pin> COMPARE_Y = (p1, p2) ->
